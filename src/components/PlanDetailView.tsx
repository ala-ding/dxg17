...
export default function PlanDetailView({ 
  ...
  // A plan is fully viewable if:
  // 1. User is Professional Member
  // 2. User is Consulting Member
  // 3. User has explicitly paid for this plan's unlock
  ...
  const handleUnlockPlan = async (tier: 'basic' | 'professional') => {
    try {
      ...
      // Persistence already handled by payOrder side effect
      onUpdate();
    } catch (e: any) {
      ...
      <div className="w-full">
        ...
        {activeTab === 'items' && (
          <div className="space-y-8 relative">
            {!isUnlocked && (
              <div className="absolute inset-x-0 bottom-0 top-[150px] z-[50] flex flex-col items-center justify-center p-12 overflow-hidden rounded-[40px] border border-white/10 bg-black/40 backdrop-blur-xl">
                 <div className="max-w-md w-full bg-[#111111] border border-white/10 p-10 rounded-[40px] text-center shadow-2xl">
                    <div className="w-20 h-20 rounded-full bg-brand/10 text-brand flex items-center justify-center mx-auto mb-8">
                       <Lock className="w-10 h-10" />
                    </div>
                    <h3 className="text-[24px] font-black text-white mb-4">解锁完整采购清单</h3>
                    <p className="text-white/40 text-[15px] font-medium mb-10">
                      当前为预览模式，解锁后可查看所有单品详情、专业采购建议，并支持一键导出清单。
                    </p>
                    
                    <div className="grid grid-cols-2 gap-4 mb-8">
                       <button 
                         onClick={() => handleUnlockPlan('basic')}
                         disabled={isPaying}
                         className="p-6 bg-white/5 border border-white/10 rounded-3xl hover:bg-white/10 transition-all text-left flex flex-col justify-between h-40 group"
                       >
                          <div>
                            <p className="text-[14px] font-black text-white mb-1">清单基础版</p>
                            <p className="text-[11px] text-white/30 font-bold">查看全部单品与价格</p>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-[20px] font-black text-white">¥29</span>
                            <div className="w-8 h-8 rounded-full bg-brand/20 text-brand flex items-center justify-center group-hover:scale-110 transition-transform">
                              <ArrowRight className="w-4 h-4" />
                            </div>
                          </div>
                       </button>
                       <button 
                         onClick={() => handleUnlockPlan('professional')}
                         disabled={isPaying}
                         className="p-6 bg-brand/10 border border-brand/20 rounded-3xl hover:bg-brand/20 transition-all text-left flex flex-col justify-between h-40 group relative overflow-hidden"
                       >
                          <div className="absolute top-0 right-0 p-2 opacity-20">
                            <Zap className="w-12 h-12 text-brand" />
                          </div>
                          <div>
                            <p className="text-[14px] font-black text-brand mb-1">清单专业版</p>
                            <p className="text-[11px] text-brand/40 font-bold font-black">含专业建议与替代推荐</p>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-[20px] font-black text-brand">¥99</span>
                            <div className="w-8 h-8 rounded-full bg-brand text-white flex items-center justify-center group-hover:scale-110 transition-transform shadow-lg shadow-brand/20">
                              <ArrowRight className="w-4 h-4" />
                            </div>
                          </div>
                       </button>
                    ...
                    <p className="text-white/20 text-[12px] font-bold">
                      咨询会员/专业会员可直接查看，无需重复支付
                    </p>
                    <button 
                      onClick={() => navigate('/membership')}
                      className="mt-4 text-[13px] font-black text-brand hover:underline"
                    >
                      了解会员权益
                    </button>
                 </div>
            ...
            <div className={`flex justify-between items-center bg-[#1A1A1A] border border-white/10 p-6 rounded-[32px] ${!isUnlocked ? 'filter blur-sm select-none pointer-events-none' : ''}`}>
                <div className="flex items-center gap-6">
            ...
            <div className={`bg-[#1A1A1A] border border-white/10 rounded-[40px] overflow-hidden shadow-2xl ${!isUnlocked ? 'filter blur-sm select-none pointer-events-none' : ''}`}>
