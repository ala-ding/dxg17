    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-6">
            <button 
              ...
              className="w-full sm:w-auto px-10 h-14 md:h-16 bg-white text-black rounded-full text-[15px] md:text-[17px] font-bold hover:scale-105 active:scale-95 transition-all shadow-2xl"
            >
              直接预览全屋方案
            </button>
            <Link 
              ...
              className="w-full sm:w-auto px-10 h-14 md:h-16 bg-white/10 backdrop-blur-xl border border-white/20 text-white rounded-full text-[15px] md:text-[17px] font-bold hover:bg-white/20 transition-all flex items-center justify-center gap-2"
            >
          ...
          <div className="mt-10 md:mt-12 flex flex-col items-center gap-4 md:gap-6">
            <div className="text-[13px] md:text-[14px] text-white/40 font-bold flex items-center gap-2">
              不知道怎么选？
            </div>
            <div className="flex flex-wrap items-center justify-center gap-6 md:gap-8">
              ...
              >
                ...
                <span className="text-[12px] md:text-[13px] font-black text-white/60 group-hover:text-white transition-colors">按空间看</span>
              </button>
              <button 
              ...
              >
                ...
                <span className="text-[12px] md:text-[13px] font-black text-white/60 group-hover:text-white transition-colors">按预算看</span>
              </button>
              <button 
              ...
              >
                ...
                <span className="text-[12px] md:text-[13px] font-black text-white/60 group-hover:text-white transition-colors">AI 帮我选</span>
              </button>
            </div>
          </div>

          <div className="flex items-center justify-center gap-4 md:gap-8 mt-12 md:mt-16 pt-6 md:pt-8 border-t border-white/5">
             <div className="flex flex-col items-center">
                <span className="text-[20px] md:text-[24px] font-black text-white">400+</span>
                <span className="text-[10px] md:text-[11px] text-white/30 uppercase tracking-widest text-center">设计师严选单品</span>
             </div>
             <div className="w-px h-6 md:h-8 bg-white/10" />
             <div className="flex flex-col items-center">
                <span className="text-[20px] md:text-[24px] font-black text-white">5</span>
                <span className="text-[10px] md:text-[11px] text-white/30 uppercase tracking-widest text-center">预算体系层级</span>
             </div>
             <div className="w-px h-6 md:h-8 bg-white/10" />
             <div className="flex flex-col items-center">
                <span className="text-[20px] md:text-[24px] font-black text-white">0</span>
                <span className="text-[10px] md:text-[11px] text-white/30 uppercase tracking-widest text-center">设计费用支出</span>
             </div>
          </div>
        </motion.div>
        ...
        <div className="absolute bottom-6 md:bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-40">
          <span className="text-[10px] md:text-[11px] font-medium tracking-[0.3em] uppercase">Scroll</span>
      ...
      >
        <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
           ...
           <div className="w-[90vw] md:w-[85vw] h-[80vh] md:h-[75vh] relative">
              <img 
                src="https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?auto=format&fit=crop&q=80&w=2600" 
                alt="Detail"
                className="w-full h-full object-cover opacity-40 rounded-[24px] md:rounded-[40px] blur-[2px]"
              />
              <div className="absolute inset-0 bg-black/40 rounded-[24px] md:rounded-[40px]" />
           </div>
        </div>

        <div className="relative z-10 w-full max-w-[1400px] px-6 md:px-10 grid grid-cols-1 lg:grid-cols-2 lg:items-center gap-10 md:gap-20">
           <motion.div
             initial={{ opacity: 0, x: -40 }}
             whileInView={{ opacity: 1, x: 0 }}
             transition={{ duration: 1 }}
           >
              <h2 className="text-[32px] md:text-[64px] font-medium tracking-tight mb-4 md:mb-8 leading-tight">
                每一档预算，<br />
                都有清晰的空间差异。
              </h2>
              <p className="text-[16px] md:text-[20px] text-white/50 font-light leading-relaxed max-w-lg mb-8 md:mb-12">
                从基础入住到高阶设计，系统将家具、灯具、软装与预算关系可视化呈现。
              </p>
              <div className="flex flex-wrap gap-3 md:gap-4">
                 {['预算可视化', '空间效果预览', '全屋搭配逻辑'].map(tag => (
                   <span key={tag} className="px-4 md:px-6 py-2 rounded-full bg-white/5 border border-white/10 text-white/70 text-[12px] md:text-[13px] font-medium backdrop-blur-md">
                     {tag}
                   </span>
                 ))}
              </div>
           ...
           <motion.div 
             ...
             className="relative aspect-video lg:aspect-[4/3] bg-white/5 rounded-2xl md:rounded-3xl border border-white/10 backdrop-blur-2xl p-6 md:p-8 flex flex-col justify-end overflow-hidden group shadow-2xl"
           >
              ...
              <h3 className="text-xl md:text-2xl font-medium mb-2 md:mb-4">精准的落地参考</h3>
              <p className="text-[14px] md:text-[16px] text-white/40 leading-relaxed font-light">
                不只是看图，底线哥为您拆解每一个环节的成本分布，让您在开始装修前就对自己“未来的家”了如指掌。
              </p>
              
              <div className="absolute top-6 right-6 md:top-10 md:right-10 flex flex-col gap-1.5 md:gap-2">
                 {[1, 2, 3].map(i => (
      ...
      >
        ...
        <motion.div 
           ...
           className="relative z-10 text-center px-6 max-w-4xl"
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-white/60 text-[12px] font-medium mb-6 md:mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-6">
            <button 
              ...
              className="w-full sm:w-auto px-12 h-14 md:h-16 bg-white text-black rounded-full text-[15px] md:text-[17px] font-bold hover:scale-105 active:scale-95 transition-all shadow-2xl"
            >
              新建 AI 方案
            </button>
            <button 
              ...
              className="w-full sm:w-auto px-12 h-14 md:h-16 bg-white/5 backdrop-blur-xl border border-white/10 text-white rounded-full text-[15px] md:text-[17px] font-bold hover:bg-white/10 transition-all"
            >
              查看预算案例
            </button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6 md:gap-10 mt-16 md:mt-24 max-w-2xl mx-auto">
             {[
               ...
               <div key={idx} className="flex flex-col items-center gap-2 md:gap-3">
                  ...
                  <span className="text-[12px] md:text-[13px] text-white/40 font-medium">{item.label}</span>
               </div>
             ))}
          </div>
        </motion.div>
        ...
        {/* Minimal Footer */}
        <div className="absolute bottom-6 md:bottom-10 inset-x-6 md:inset-x-10 flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] md:text-[11px] font-medium tracking-widest text-white/20 uppercase text-center">
      ...
      {/* Nav dots - Hidden on mobile to avoid clutter */}
      <div className="hidden md:flex fixed right-8 top-1/2 -translate-y-1/2 z-[150] flex flex-col gap-4">
        {[0, 1, 2, 3].map(i => (
          <button 
            key={i}
