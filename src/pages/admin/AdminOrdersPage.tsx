        ...
        <div className="bg-[#141414] border border-white/5 rounded-[40px] overflow-hidden shadow-2xl">
           <table className="w-full text-left border-collapse">
              <thead className="bg-white/[0.02] border-b border-white/5">
                 <tr>
                    <th className="px-8 py-6 text-[12px] font-black text-white/20 uppercase tracking-widest">订单/类型/客户</th>
                    <th className="px-8 py-6 text-[12px] font-black text-white/20 uppercase tracking-widest">金额/支付</th>
                   ...
                   <tr key={o.id} className="border-b border-white/5 hover:bg-white/[0.01] transition-all">
                      <td className="px-8 py-8">
                         <div className="flex items-center gap-5">
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${o.type === 'membership' ? 'bg-brand/10 text-brand' : o.type === 'plan_list_unlock' ? 'bg-blue-500/10 text-blue-500' : 'bg-white/5 text-white/40'}`}>
                               {o.type === 'membership' ? <ShieldCheck className="w-6 h-6" /> : o.type === 'plan_list_unlock' ? <Lock className="w-6 h-6" /> : <Package className="w-6 h-6" />}
                            </div>
                            <div>
                               <p className="text-[16px] font-black text-white group-hover:text-brand transition-colors">{o.order_no}</p>
                               <p className="text-[12px] text-brand font-bold uppercase tracking-widest mt-0.5">
                                  {o.type === 'membership' ? '会员下单' : o.type === 'plan_list_unlock' ? '清单解锁' : o.type === 'service_order' ? '服务下单' : '产品下单'}
                                  {o.title && <span className="text-white/40"> · {o.title}</span>}
                               </p>
                               <p className="text-[13px] text-white/40 font-bold mt-1">{o.customer_name || '访客屋主'}</p>
                            </div>
                         </div>
                      </td>
                      <td className="px-8 py-8">
                         <p className="text-[18px] font-black text-brand mb-1">¥{pricing.formatCurrency(o.amount || o.grand_total || 0)}</p>
                         <p className={`text-[11px] font-black uppercase tracking-widest ${o.payment_status === 'paid' ? 'text-emerald-500' : 'text-white/20'}`}>
                           {o.payment_status === 'unpaid' ? '未支付' : o.payment_status === 'deposit_paid' ? '已付定金' : o.payment_status === 'paid' ? '已支付' : o.payment_status || '未知'}
                         </p>
                      </td>
                      <td className="px-8 py-8">
                         <div className="flex flex-col gap-2">
                           <span className={`px-4 py-1 border border-white/5 rounded-lg text-[11px] font-black uppercase tracking-wider inline-block w-fit ${o.payment_status === 'paid' || o.status === 'paid' ? 'bg-brand/10 text-brand' : 'bg-white/5 text-white/60'}`}>
                             {getStatusLabel(o.status)}
                           </span>
                           {(o.type === 'product_order' || !o.type) && (
